...
點擊下方連結開啟網頁 app，<mark>推薦使用手機版本<mark>。

Promoradar 連結：**https://promoradar.vercel.app/**

一般使用者 (user) 帳號操作流程：
1. 進到網頁 app 之後預設顯示儀表版，可以在不登入的情況下瀏覽所有優惠活動。
2. 欲執行收藏關注等操作，會自動跳至登入頁面，需先行註冊與登入方能操作。

品牌經營者/管理者 (admin) 帳號操作流程：
1. 進到網頁 app 之後預設顯示儀表版，可以在不登入的情況下瀏覽所有優惠活動。
2. 點擊右上角三橫 -> 登入/註冊 -> 輸入以下 admin 帳密：
   - 電子郵件：demo@gmail.com
   - 密碼：demo1234
   
   (google 有可能會顯示密碼已洩漏，因該帳密僅作測試用，請忽略安全警示。)
3. admin 登入後，右下角會比一般使用者多出"Admin"分頁，為 KFC 的品牌管理後台，可對 KFC 相關資料執行 admin 限定操作。
